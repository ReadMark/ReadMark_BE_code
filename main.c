...
char url[128] = "http://43.200.102.14:5000/api/upload/esp32-cam/upload"; // 서버 접속 URL
...
static camera_config_t camera_config = {
    ...
    .fb_location = CAMERA_FB_IN_PSRAM,   // PSRAM 사용
...
int sendPhoto(const char *url, char *resp_buf, size_t resp_buf_sz)
{
    ...
    {
        ESP_LOGI(sendPhotoTag, "서버 URL : %s", url);
        ESP_LOGI(sendPhotoTag, "로컬 IP: " IPSTR ", 게이트웨이: " IPSTR, 
    ...
    esp_http_client_config_t cfg = {
        ...
        .timeout_ms = 30000,        // 타임아웃 시간 증가 (10초 → 30초)
    ...
    ESP_LOGI(sendPhotoTag, "서버 연결 시도 중...");
    esp_err_t err = esp_http_client_open(client, content_len);

    if (err != ESP_OK)
    {
        ESP_LOGE(sendPhotoTag, "서버 연결 실패: %s (0x%x)", esp_err_to_name(err), err);
        ESP_LOGE(sendPhotoTag, "가능한 원인:");
        ESP_LOGE(sendPhotoTag, "  1. 서버가 실행 중이 아닐 수 있음");
        ESP_LOGE(sendPhotoTag, "  2. 네트워크 방화벽/라우팅 문제");
        ESP_LOGE(sendPhotoTag, "  3. 서버 주소/포트가 잘못되었을 수 있음");
        goto WRITE_FAIL; // 클라이언트 핸들 정리 (소켓 닫기, 내부 버퍼, 메모리 해제)
    }
    
    ESP_LOGI(sendPhotoTag, "서버 연결 성공!");
